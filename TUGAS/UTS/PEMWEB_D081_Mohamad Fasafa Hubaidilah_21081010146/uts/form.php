    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
          ...
          <h2 style="margin: 30px 0 30px 0;">Form <?php echo ucwords(preg_replace("(_)", " ", $table)) ?></h2>
          <form action="" method="POST">
              ...
              // foreach($kolom as $atribut){ 
                ...
                if($rowCek != NULL){ ?>
                  <div class="form-group">
                    <label><?php echo ucwords(preg_replace("(_)", " ", $row['COLUMN_NAME'])); ?></label>
                    <select class="custom-select" name="<?php echo $row['COLUMN_NAME']; ?>" required="required">
                      ...
                      <?php 
                        ...
                        $qGetSecondColumn = "SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$dbName' AND TABLE_NAME='$rowCek[REFERENCED_TABLE_NAME]' AND ORDINAL_POSITION=2";
                        $rGetSecondColumn = mysqli_query(connection(),$qGetSecondColumn);
                        $rowGetSecondColumn = mysqli_fetch_array($rGetSecondColumn);
                        while ($rowDropdown = mysqli_fetch_array($rDropdown)) {?>
                          <option value="<?php echo $rowDropdown[$rowCek['REFERENCED_COLUMN_NAME']]; ?>" ><?php echo $rowDropdown[$rowCek['REFERENCED_COLUMN_NAME']].". ".$rowDropdown[$rowGetSecondColumn['COLUMN_NAME']]; ?></option>
                        <?php } ?>
                    </select>
                  </div>
              <?php } else{ ?>
            ...
            <?php if($row['DATA_TYPE'] == "text" || $row['DATA_TYPE'] == "mediumtext"){ ?>
                <textarea rows="5" class="form-control" name="<?php echo $row['COLUMN_NAME']; ?>" <?php echo $row['IS_NULLABLE'] == "NO" ? "required" : "" ?> placeholder="<?php echo ucwords(preg_replace("(_)", " ", $row['COLUMN_NAME'])); ?>"></textarea>
              <?php }else{?>
                <input type="<?php echo $row['DATA_TYPE'] == 'decimal' || $row['DATA_TYPE'] == 'smallint' || $row['DATA_TYPE'] == 'int' ? 'number' : ($row['DATA_TYPE'] == 'date' ? 'date' : 'text') ?>" class="form-control" placeholder="<?php echo ucwords(preg_replace("(_)", " ", $row['COLUMN_NAME'])); ?>" name="<?php echo $row['COLUMN_NAME']; ?>" <?php echo $row['IS_NULLABLE'] == "NO" ? "required" : "" ?> <?php echo isset($_GET[$row['COLUMN_NAME']]) ? "readonly" : "" ?>>
              <?php } ?>
            </div>
            <?php }} ?>
